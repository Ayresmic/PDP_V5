	...
	*/
	...
	displayDeliveryDateMsg:function(resolvedItem, itemDeliveryDate){
		...
		if(cutoffTime.getHours() > 12){
			var time = cutoffTime.getHours() - 12;
			document.getElementById("orderdate").innerHTML = time + "pm " +  dateString;
		}else{
			document.getElementById("orderdate").innerHTML = cutoffTime.getHours() + "am " +  dateString;
		}
...
function persistentPDP(){
	...
	var persistentCTA = document.getElementById("pdp-persistent-container");
	var placeholder = document.getElementById("pdp-persistent-placeholder");
	var persistentTrigger = document.getElementById("pdp-persistent-trigger");	
	...
	if (!isMobile) {
		...
		if (scrollTop <= persistentTrigger.offsetTop + bottomPromoOffsetHeight - header.offsetHeight){			
...
function persistentReplacementPDP(stickyOffset){
	...
	if($("#pdp-persistent-spacer").length ==0){
		...
		$("#pdp-persistent-spacer").height($("#pdp-persistent-wrapper").height());
	}
	...
	if (!isMobile) {
	  ...
	  var sticky = $(persistentCTA),
		...
		if(scroolOfPageMobile < (scrollHeightMobile+heightOfConatner)){
			...
			$("#pdp-persistent-spacer").append($("#pdp-persistent-wrapper"));
...
function setUpPersistentPDP(){
	if (document.querySelectorAll(".pdp-fixed-product-info")[0] && document.querySelectorAll(".pdp-product-features")[0]){
		var parent = document.querySelectorAll('.pdp-product-right #repeat-delivery-select')[0],
			...
			newAnchorWrapper = document.createElement("div"),
		...
		newAnchorWrapper.setAttribute("id", "pdp-persistent-trigger");
		...
		$(newAnchorWrapper).insertAfter(newPlaceholder);
		var footer = document.querySelectorAll('.footer_wrapper_position')[0];
		$(footer).prepend(newFooterSpacer);
